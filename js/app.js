...
function renderLoanEdit(){
  ...
  h+='<div class="form-group"><label>还款方式</label><select class="f-input" id="f_planMethod" onchange="togglePlanFreq()"><option value="lump" '+(f.planMethod==='lump'?'selected':'')+'>一次性还本付息</option><option value="equalInstallment" '+(f.planMethod==='equalInstallment'?'selected':'')+'>等额本息</option><option value="equalPrincipal" '+(f.planMethod==='equalPrincipal'?'selected':'')+'>等额本金</option></select></div>'
...
function renderLoanDetail(){
  ...
  if(raw.planMethod&&raw.planMethod!=='lump'){
    var plan=c.plan;var planLabel={'equalInstallment':'等额本息','equalPrincipal':'等额本金'}[raw.planMethod]||''
...
function renderReminder(){
  ...
  h+='<div class="card"><div class="card-title">还款能力分析</div><div class="stat-row2"><div class="stat-box"><div class="stat-label">待还总额</div><div class="stat-val red">'+money(pendingTotal)+'</div></div><div class="stat-box"><div class="stat-label">待入账收入</div><div class="stat-val green">'+money(incomeTotal)+'</div></div></div>'
  if(incomeTotal>0){
